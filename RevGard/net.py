...
class ResNet50Fc(BaseFeatureExtractor):
    ...
    ** input image should be in range of [0, 1]**
    ...
    def __init__(self,model_path=None, normalize=True):
        ...
        self.layer5 = nn.Linear(2048, 256)   # bottleneck layer
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.layer5(x)                 # bottleneck layer
        return x
...
class AlexnetFc(BaseFeatureExtractor):
    def __init__(self,model_path=None, normalize=True):
        ...
        if model_path:
            if os.path.exists(model_path):
                self.model_alex = models.alexnet(pretrained=False)
                self.model_alex.load_state_dict(torch.load(model_path))
            else:
                raise Exception('invalid model path!')
        else:
...
class CLS(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, bottle_neck_dim=256):
        ...
        self.bottleneck = nn.Linear(256, bottle_neck_dim) 
